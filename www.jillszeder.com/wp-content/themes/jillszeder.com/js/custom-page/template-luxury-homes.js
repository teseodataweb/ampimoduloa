	...
	idxb_properties : function(){
		...
		if(jQuery('.IDX-resultsCell').length){
			jQuery('.IDX-resultsCell').each( function(){
				...
				var img = jQuery(this).find('.IDX-resultsPhotoImg').attr('src').trim();
		        ...
		        '<div class="fl-prop">'+
					'<a href="'+link+'&chkpopup=true">'+
						'<div class="fl-img">'+
							'<canvas width="488" height="349" style="background-image: url(https://imageproxy.agentimage.com/488x349/'+img+');"></canvas>'+
						'</div>'+
						'<div class="fl-det">'+
							'<div class="fl-name">'+
								city + soldStat +
		...
		jQuery('select.idxb-sort').on('change',function(){
			...
			var start = GetURLParameter('start');
		  	...
		  	if(GetURLParameter('start') == undefined){
				newLocation = location.origin + location.pathname + '?per=' + per + '&srt=' + valueSelected  + '#fl-wrap';
			}
			else{
				newLocation = location.origin + location.pathname + '?per=' + per + '&start=' + start + '&srt=' + valueSelected + '#fl-wrap' ;
			}
